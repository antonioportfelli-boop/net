...
test("the auth schema ships outside the globbed directory", () => {
  ...
  // Auth stays under migrations/auth/ (not globbed). steel_rows may sit in the
  // top-level migrations/ dir and correctly appear as pending.
  const pending = pendingMigrations(readdirSync(migrationsDir), []);
  assert.ok(!pending.some((m) => m.name === AUTH_MIGRATION));
  assert.deepEqual(pending, [
    { name: "0002_steel_rows.sql", path: "0002_steel_rows.sql" },
  ]);
