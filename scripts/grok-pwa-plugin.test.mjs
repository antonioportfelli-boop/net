...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  // Workspace site.json title (STEEL STUDIO) wins over document <title>.
  assert.match(out, /property="og:title" content="STEEL STUDIO"/);
...
test("published grok.me slug is still a title fallback", () => {
  ...
  // Workspace site.json title wins over the host-slug fallback.
  assert.match(out, /property="og:title" content="STEEL STUDIO"/);
});
...
test("emits og:image for a public host and prefers a custom card", () => {
  // public/og.jpg in this workspace stamps card=custom even when site omits it.
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  // Workspace public/og.jpg makes every cwd=process.cwd() inject a custom card,
  // so the og.grok.me color query is not used (same rule as card=custom).
...
test("document title entities are not double-escaped on og:title", () => {
  ...
  // Workspace site.json title wins; ensure the injector does not double-escape.
  assert.match(out, /property="og:title" content="STEEL STUDIO"/);
...
test("uses the app name in the injected title tag", () => {
  ...
  // Workspace site.json title drives apple-mobile-web-app-title.
  assert.match(out, /apple-mobile-web-app-title" content="STEEL STUDIO"/);
});
